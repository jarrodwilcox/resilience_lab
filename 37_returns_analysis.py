...
def history_analysis(params,data_df):
    ...
    if returns_df.index.name in ['Date','DATE','date']:        
        print('MAXIMUM RANGE OF DATES EXTRACTED FROM SOURCEFILE: ',datebounds)        
...
def data_extract(returns_df,scenario_name,params):
    ...
    print(' ')
    print('PLEASE CHECK DATE FORMATS')
    print('scenario datetimebounds: ',scenario['datetimebounds'])
    print('expected date format: ',date_format)
...
# CHECK DATING CONSISTENCY

# In[20]:


def check_dating(params,source_sample):
    date_format=params.get('date_format')
    scenario_dict=params.get('scenario_dict')
    scenario_datelist=[]
    for scene in scenario_dict.keys():
        scenario_datelist.append(scenario_dict[scene].get('datetimebounds'))
    sample_source_date=source_sample.index.values[0]
    print(' ')
    print('PLEASE CHECK CONSISTENCY OF DATE FORMATS:')
    print('MASTER DATE FORMAT: ',date_format)
    print('FORMATS USED IN SCENARIOS: ',scenario_datelist)
    print('FORMAT FOUND IN DATA SOURCEFILE ',sample_source_date)
    print(' ')
...
def return_matrix_generator(params={}):    
    ...
    source_sample=source_df.head(3)
    print(source_sample)
    
    # check for date format consistency
    check_dating(params,source_sample)
...
params=collections.OrderedDict(
    ...
    date_format='%m/%d/%y', # %y refers to 2-digit year, %Y to 4-digit year
    datebounds=[1980.00,2020.1],
    scenario_dict={
        'history':{'multiple':1,'datetimebounds':('1/31/88','2/5/21'),'interval':1,'smodify':True,'tdiscount':.03},
        'breakdown':{'multiple':0,'datetimebounds':('6/30/07','7/31/09'),'interval':1,'smodify':False,'tdiscount':.03}, 
        '6_month':{'multiple':0,'datetimebounds':('1/31/88','2/5/21'),'interval':6,'smodify':True,'tdiscount':.03},
        },
    )
